...
public:
    ...
    struct KD_TREE_NODE
    {
        PointType point;
        int division_axis;
        int TreeSize = 1;          // 字节点数量
        int invalid_point_num = 0; // 子树上无效节点的数量
        int down_del_num = 0;
        bool point_deleted = false; // 当前节点被删除
        bool tree_deleted = false;  // 整个子树被删除
        ...
        float node_range_x[2], node_range_y[2], node_range_z[2]; // 子树的空间范围
        float radius_sq;
