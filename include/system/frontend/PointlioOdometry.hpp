...
public:
    ...
    virtual bool run(bool map_update_mode, PointCloudType::Ptr &feats_undistort)
    {
        ...
        if (!map_update_mode && ikdtree.Root_Node == nullptr)
        {
            ...
            {
                if (use_imu_as_input)
                    pointcloudLidarToWorld(feats_undistort, feats_down_world, state_in);
                else
                    pointcloudLidarToWorld(feats_undistort, feats_down_world, state_out);
    ...
    virtual void set_pose(const state_ikfom &cur_state)
    {
        ...
        {
            state_in = kf_input.get_x();
            state_in.pos = cur_state.pos;
            state_in.rot = cur_state.rot;
            kf_input.change_x(state_in);
        }
        else
        {
            state_out = kf_output.get_x();
            state_out.pos = cur_state.pos;
            state_out.rot = cur_state.rot;
            kf_output.change_x(state_out);
        }
    }
