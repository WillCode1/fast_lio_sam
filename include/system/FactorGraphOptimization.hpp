...
private:
    ...
    void add_gnss_factor()
    {
        ...
        if (gnss->get_gnss_factor(thisGPS, this_pose6d.time, this_pose6d.z))
        {
            ...
            gtsam::GPSFactor gps_factor(keyframe_pose6d_optimized->size(), gtsam::Point3(thisGPS.position(0), thisGPS.position(1), thisGPS.position(2)), gnss_noise);
            gtsam_graph.add(gps_factor);
            loop_is_closed = true;
            LOG_WARN("GPS Factor Added, noise = (%f, %f, %f).", Vector3(0), Vector3(1), Vector3(2));
        }
    }
